...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #   action node is added, it MUST be connected to the S node instances in the appropriate s_level set.p
        #literal.symbol:Have(Cake)  literal.is_pos:True
        #literal.symbol:Eaten(Cake) literal.is_pos:False
        #action.name:Eat #action.precond_pos:[Have(Cake)] #action.precond_neg:[]

        self.a_levels.append(set()) 
        #from github ,ans needed to add but do not know why, now understand 
        
        for action in self.all_actions:
            length = 0
            len_a_pos = len(action.precond_pos)
            len_a_neg = len(action.precond_neg)
            act = PgNode_a(action)  
            for literal in self.s_levels[level]: 
               if (literal.is_pos == True) and (literal.symbol in action.precond_pos):
                   length = length + 1 
               if (literal.is_pos == False) and (literal.symbol in action.precond_neg):
                   length = length + 1          
            if (length == len_a_pos + len_a_neg):    
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level-1]:
            for literal in action.effnodes:
               lit = PgNode_s(literal.symbol, literal.is_pos) 
               if leng == 0:
                  self.s_levels[level].add(lit)
                  leng = 1
               else:
                   for s in self.s_levels[level]:
                      if(((literal.symbol == s.symbol) and (literal.is_pos == s.is_pos)) != True):
                         can = 1
                      else:
                         can = 0
                   if (can == 1):
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        l_a = []
        l_b = []
        for l in node_a1.effnodes:
           l_a.append(l)
          
        for l in node_a2.effnodes:
           l_b.append(l)
        
        for la in l_a:
           for lb in l_b:
              if (la.symbol == lb.symbol and la.is_pos != lb.is_pos):
                  return True
        return False
        ...
        for p in node_a1.prenodes:
           for a in node_a2.effnodes:
              if (p.is_pos == True):
                ...
                negation = True
              if (negation != a.is_pos and p.symbol == a.symbol):
                 return True
        
        negation = True 
        for p in node_a2.prenodes:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        #learn from github answer: action.precond_pos,action.precond_neg is not same as parents from PgNode_a
        # TODO test for Competing Needs between nodes
        pre1 = node_a1.parents
        pre2 = node_a2.parents
        
        for p1 in pre1:
           for p2 in pre2:
              if (p1.is_mutex(p2) == True):
                 return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        #is parents not parent from github answer
        # TODO test for Inconsistent Support between nodes
        #read github ans code, to understand inconsistent_support_mute.i write code individually
        pact1 = node_s1.parents
        pact2 = node_s2.parents
        for p1 in pact1:
          for p2 in pact2:
             if (p1.is_mutex(p2) == False):
                return False
        return True
